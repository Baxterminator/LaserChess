...
bool ShiftMove::LegalMove(const Board_t& board, PieceColor playerColor) const {
  if (!ValidateMove(this->StartPosition, this->EndPosition) && ValidatePosition(this->StartPosition) &&
      ValidatePosition(this->EndPosition)) {
    return false;
  }

  Square startSq = board[this->StartPosition.y][this->StartPosition.x];
  Square endSq = board[this->EndPosition.y][this->EndPosition.x];
  PieceColor oppositeColor = (playerColor == PieceColor::BLUE) ? PieceColor::RED : PieceColor::BLUE;
...
bool SwitchMove::LegalMove(const Board_t& board, PieceColor playerColor) const {
  if (!ValidateMove(this->StartPosition, this->EndPosition) && ValidatePosition(this->StartPosition) &&
      ValidatePosition(this->EndPosition)) {
    return false;
  }
  ...
  if ((startSq.Piece == nullptr) || (endSq.Piece == nullptr) || (startSq.Piece->GetPieceType() != PieceType::SWITCH)) {
    return false;
  }
...
void SwitchMove::ApplyMove(Board& board) {
  ...
  std::unordered_set<Square, Square::HashFunction>& startPieceSquares =
      (startPiece->GetPieceColor() == PieceColor::BLUE) ? board.BluePiecesSquares : board.RedPiecesSquares;
  std::unordered_set<Square, Square::HashFunction>& endPieceSquares =
      (endPiece->GetPieceColor() == PieceColor::BLUE) ? board.BluePiecesSquares : board.RedPiecesSquares;
