...
void GameServer::waitForPlayers() {
  ...
  player1 = Player(Player::PLAYER_1, game::PieceColor::BLUE, wait_for_client());
  player1.sendLevel(lvl);

  std::cout << "Waiting for Player 1" << std::endl;
  player2 = Player(Player::PLAYER_2, game::PieceColor::RED, wait_for_client());
  player2.sendLevel(lvl);
}
